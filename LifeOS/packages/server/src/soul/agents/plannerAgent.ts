...
规则：
1. confidenceScore: 即“你对整个战略规划”的综合置信度。如果你觉得给出的建议很敷衍或者不确定，给出低分。但一旦发现“原始转写/文本捕获/原始文字”里的待办，请大胆给出高置信度。
2. suggestedActions 的 kind 只能从以下选择：
   - "extract_tasks": 笔记中（特别是“原始转写”或“文本捕获”段落里）有明确或隐含的待办安排或琐事需要提取
