...
4. suggestedActions 的 kind 只能从以下选择：
   ...
   - "multiple_choices": 当针对高风险或缺乏明确信息的动作做规划时，生成两个选项（如 Plan A vs Plan B）。reason 字段里给出不同选项的具体描述，让用户授权二选一。
...
const AI_SUGGESTABLE_ACTION_KINDS: SoulActionKind[] = ['extract_tasks', 'update_persona_snapshot', 'ask_followup_question', 'persist_continuity_markdown', 'dispatch_physical_action', 'multiple_choices'];
