...
### 背景
灵光 App 的数据需要暂存后由 OpenClaw 整理。方案 A 是建立独立的 Vault_LingGuang 子目录，方案 B 是复用 _Inbox 目录。

### 决策
采用方案 B — 所有输入端的数据统一写入 `_Inbox/`，通过 `source` 字段区分来源。

### 理由
- LifeOS 的 classify-inbox 功能已适配 `_Inbox`
- OpenClaw 只需监控一个目录
- 通过 frontmatter `source: lingguang` 即可区分来源
- 独立目录会导致 LifeOS 索引器需额外排除逻辑
...
### 决策
灵光端只确定 `type`（通过 VoiceNoteType 映射），`dimension` 统一填 `_inbox`，分类交给 OpenClaw。

### 理由
- 手机端 AI 调用慢、成本高
- 灵光端上下文信息少，分类不准
- OpenClaw 有完整的 Vault 上下文，分类更准确
- 减少灵光端的复杂度和 AI 调用量

### 影响
- 灵光端 AI prompt 不需要判断维度
- OpenClaw 必须实现自动分类功能（L1 能力）

---

## ADR-004: LifeOS 负责「看」，OpenClaw 负责「做」

**日期**: 2026-03-17
**状态**: 已采纳 ✅

### 背景
LifeOS 已内置 AI 分类和行动项提取功能（Phase 5），OpenClaw 也计划做类似的事情，存在职责重叠。
...
### 影响
- LifeOS 现有的 classify-inbox 和 extract-tasks API 可保留为手动触发入口
...
### 决策
- **LifeOS** 成为主脑 / orchestration layer，负责判断是否需要外部执行、创建任务请求、跟踪状态、校验结果、落地笔记并触发既有索引闭环。
- **OpenClaw** 只负责执行明确、有限、可描述的任务，并返回结构化结果。
- 没有 LifeOS 持有的任务记录，不允许系统凭空生成最终结果笔记。
- `classify-inbox`、`extract-tasks` 等旧 `/api/ai/*` 能力降级为 legacy/manual tools，不再代表主架构。
...
### 背景
ADR-007 确立了 LifeOS 编排 + OpenClaw 按需 worker 的架构。但实际运行中发现，OpenClaw 的 4 个 cron 任务（scan-inbox、auto-exec、daily-report、weekly-report）已关闭，而 LifeOS 后端已具备完整的 AI 基础设施（`callClaude()`、`classifyNote()`、`extractTasks()`），完全可以直接完成这些自动化任务，无需绕道 OpenClaw。

### 决策
LifeOS 后端新增 3 种 worker task 类型，全部由 LifeOS 直接调用 Claude AI 完成：
- `classify_inbox` — 扫描 `_Inbox/` 并自动分类归档（替代 scan-inbox）
- `daily_report` — 生成每日回顾（替代 daily-report）
- `weekly_report` — 生成每周回顾（替代 weekly-report）
