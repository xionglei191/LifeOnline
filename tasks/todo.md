...
- 本轮验证再补充：
  ...
  - 当前环境仍有 Node engine warning：包声明要求 `>=20 <21`，实际为 `v25.8.1`，但本轮测试与构建均通过。
- 本轮继续完成的真实实现：
  - `LifeOS/packages/server/test/reintegrationApi.test.ts` 新增 defer / discard 的 websocket contract test，锁定 soul action 在 `deferred` / `discarded` 两条治理分支上也会广播 `soul-action-updated`，且事件中的 `sourceNoteId`、`sourceReintegrationId`、`governanceStatus`、`executionStatus`、`deferredAt`、`discardedAt` 与 detail/list 读回结果保持一致。
  - 这次补的是新的 server-side contract gap：此前 approve / dispatch / accept 已有 websocket 回归保护，但 defer / discard 仍缺少“事件源与 follow-up 查询同源收敛”的可回归锚点，web 侧若后续消费这两个治理状态，会对实时刷新语义形成隐性假设；本轮先在 server contract 层把这条事实源一致性补齐。
- 本轮验证再补充：
  - `pnpm --dir "/home/xionglei/LifeOnline/LifeOS/packages/server" exec node --import tsx --test --test-name-pattern "soul-action defer and discard websocket events stay aligned with detail and list views" test/reintegrationApi.test.ts` 通过。
  - `pnpm --dir "/home/xionglei/LifeOnline/LifeOS/packages/server" exec node --import tsx --test test/reintegrationApi.test.ts` 通过。
- 当前未完成项再补充：
  - defer / discard 虽已有 server + shared contract，但 settings 侧还未真实消费这两个治理动作；如果后续要开放 UI，需要优先做最小 contract-to-UI 投射，而不是再堆同类 grouped 收敛测试。
  - 本轮 server 测试变更待提交 git commit。
- 本轮继续完成的真实实现再补充：
