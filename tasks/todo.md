# 当前轮：DimensionView websocket 收口
...
## 计划
- [x] 在不覆盖并行 dirty 文件的前提下，沿新的事实源一致性问题推进 AI 洞察面板刷新竞态收口。
- [x] 让 `AISuggestions.vue` 只接受最新刷新请求的回写结果，避免旧洞察请求晚返回或晚报错时覆盖当前面板状态。
- [x] 补最小组件回归，锁定旧刷新请求晚返回/晚失败时不会把当前洞察和错误态拉回过期状态。
- [x] 运行 focused web 验证并在通过后提交。
...
## 计划
- [x] 在不覆盖并行 dirty 文件的前提下，沿新的事实源一致性问题推进 dimension 并发请求回写保护收口。
- [x] 让 `useDimensionNotes.ts` 只接受最新维度请求的回写结果，避免旧维度请求晚返回时覆盖当前页面状态。
- [x] 补 composable focused 回归，锁定旧维度请求晚返回或晚报错时不会把新维度 notes/error 覆盖回去。
- [x] 运行 focused web 验证并在通过后提交。

## 当前执行
- 已确认当前工作树并行改动仍为：`CLAUDE.md`、`LifeOS/packages/server/config.json`、`lifeonline-claude-worker-v2.sh`、`LifeOS/packages/web/src/components/TimelineTrack.test.ts`、`LifeOS/packages/web/src/composables/useCalendar.test.ts`、`LifeOS/packages/web/src/composables/useTimeline.test.ts`、`LifeOS/packages/web/src/views/CalendarView.test.ts`、`LifeOS/packages/web/src/views/DimensionView.test.ts`、`LifeOS/packages/web/src/views/SearchView.test.ts`、`LifeOS/packages/web/src/views/StatsView.test.ts`。本轮未覆盖这些无关文件。
- 本轮完成的真实实现：
  - `LifeOS/packages/web/src/composables/useDimensionNotes.ts`
    - 为每次 `load()` 分配递增 request id，只允许最新维度请求写回 `notes/error/loading`。
    - 修复快速切换 dimension 或 websocket reload 重叠时，旧维度请求晚返回仍可能把当前页面回写成旧维度结果的真实风险。
  - `LifeOS/packages/web/src/composables/useDimensionNotes.test.ts`
    - 新增最新维度成功后旧请求晚返回不会覆盖 notes 的回归。
    - 新增最新维度成功后旧请求晚失败不会错误污染页面 error 状态的回归。
- 这次修的不是补一个同类 view 对称测试，而是修复 dimension 主路径里的真实竞态：路由已经切到新维度，但旧请求后返回仍可能把页面拉回过期维度事实源。

## 本轮选择依据
- 这是新的事实源一致性问题：当前 route dimension 已变化后，旧请求结果仍能回写，会让 UI 重新退回过期维度状态。
- 这属于真实用户可见行为缺口，尤其在快速切换维度或 websocket 刷新叠加时会直接出现错位。
- 这条线与 timeline/calendar stale-request guard 一致，但作用点是新的 dimension 主路径，不是低边际同类平移测试。

## 本轮验证
- 已通过：`cd "/home/xionglei/LifeOnline/LifeOS/packages/web" && NODE_OPTIONS="--max-old-space-size=4096" npx vitest run --pool vmThreads src/composables/useDimensionNotes.test.ts`

## 当前未完成项
- 本轮改动尚未提交 git commit。
- 若继续沿同一主线推进，后续可再检查 `SearchView` 是否存在 query 快速变化时旧请求回写覆盖新结果的同类竞态，但这还未开始。

## 下一步建议
- 提交本轮 focused commit，只包含 dimension stale-request guard 收口相关文件。
...
- [x] 继续把 `DimensionHealth` / `NoteDetail` 迁移到 shared/web dimension helper，并补最小 web 回归。

## 当前执行
- 已确认当前工作树并行改动仍为：`CLAUDE.md`、`LifeOS/packages/server/config.json`、`lifeonline-claude-worker-v2.sh`。本轮未覆盖这些无关文件。
- 本轮完成的真实实现：
  - `LifeOS/packages/shared/src/types.ts`
    - 新增 `DIMENSION_LABELS`、`SELECTABLE_DIMENSIONS`、`SelectableDimension`、`DIMENSION_DIRECTORY_NAMES`、`DIMENSION_KEY_BY_DIRECTORY`，把维度标签/可选集合/目录映射提升为 shared contract。
  - `LifeOS/packages/server/src/utils/dimensions.ts`
    - 改为直接复用 shared 维度 contract，移除 server 侧独立维护的标签/目录映射常量。
  - `LifeOS/packages/web/src/components/CreateNoteFab.vue`
    - 创建笔记维度下拉改为复用 shared `SELECTABLE_DIMENSIONS` + `DIMENSION_LABELS`。
  - `LifeOS/packages/web/src/utils/dimensions.ts`
  ...
  - `LifeOS/packages/web/src/components/DimensionHealth.vue`
    - 八维度生命矩阵的维度标签与颜色改为复用 shared/web dimension helper。
  - `LifeOS/packages/web/src/components/NoteDetail.vue`
    - 笔记详情页头和 OpenClaw 结果归档维度下拉改为复用 shared/web dimension helper。
  - `LifeOS/packages/server/test/dimensions.test.ts`
  ...
  - `LifeOS/packages/web/src/components/DimensionHealth.test.ts`
    - 新增八维度矩阵回归，锁定卡片维度标签和颜色来源于 shared helper。
  - `LifeOS/packages/web/src/components/NoteDetail.test.ts`
    - 新增笔记详情维度展示/下拉回归，锁定 note header 与 worker 维度选项来源于 shared helper。
  - `LifeOS/packages/web/src/components/AISuggestions.test.ts`
...
## 本轮选择依据
- 这是新的事实源一致性问题：维度集合虽然已在 shared 里有 union type，但标签、可选集合和目录映射仍分散在 server 与多个 web 组件里重复维护。
- 维度语义会直接影响新建笔记、目录路由、统计展示和 worker 输出归档，属于真实行为契约，而不是单纯展示文案。
- 相比继续做 grouped governance / SettingsView 同类收敛，这条线更符合当前优先级中的“新的事实源一致性问题”。
...
- `pnpm --dir "/home/xionglei/LifeOnline/LifeOS" --filter web test -- src/components/DimensionHealth.test.ts src/components/NoteDetail.test.ts` 通过；受 vitest 配置影响，同时跑过现有 web 测试集，13 files / 147 tests 全通过。
- 当前环境仍有既有 Node engine warning（声明 `>=20 <21`，实际 `v25.8.1`），但未影响本轮验证。

## 当前未完成项
- 本轮改动尚未提交 git commit。
- 仓库中其它维度标签重复点后续仍可继续迁移到 shared helper，优先 `TimelineTrack.vue`、`StatsView.vue`、`WeeklyHighlights.vue`、`NotePreview.vue`、`TimelineView.vue` 等仍残留重复维度语义的主路径组件。
